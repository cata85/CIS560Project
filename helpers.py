...
def insert_initial_values(conn, handler, INIT):
    for game in INIT['Games']:
        ...
        cards = list((int(card['GameID']), card['Name'], card['Type'], card['State']) for card in INIT['Cards'][game_id])
        players = list((int(player['GameID']), player['PlayerName']) for player in INIT['Players'][game_id])
        tiles = list((int(tile['GameID']), tile['Name'], tile['Floor'], tile['State']) for tile in INIT['Tiles'][game_id])
    ...
    monsters = list((int(monster['TileID']), monster['Name']) for monster in INIT['Monsters'])
    items = list((int(item['TileID']), item['Name']) for item in INIT['Items'])
